...
$perpage = optional_param('perpage', DIPLOMA_PER_PAGE, PARAM_INT);

// Ensure the perpage variable does not exceed the max allowed if
// the user has not specified they wish to view all diplomas.
if (DIPLOMA_PER_PAGE !== 0) {
    if (($perpage > DIPLOMA_MAX_PER_PAGE) || ($perpage <= 0)) {
        $perpage = DIPLOMA_MAX_PER_PAGE;
    }
} else {
    $perpage = '9999999';
}

$url = new moodle_url('/mod/diploma/report.php', array('id'=>$id, 'page' => $page, 'perpage' => $perpage));
if ($download) {
...
if (!$diploma = $DB->get_record('diploma', array('id'=> $cm->instance))) {
    print_error('diploma ID was incorrect');
}
...
$strdiploma  = get_string('modulename', 'diploma');
...
if (!$download) {
    ...
    $PAGE->set_title(format_string($diploma->name).": $strreport");
...
add_to_log($course->id, 'diploma', 'view', "report.php?id=$cm->id", '$diploma->id', $cm->id);

// Ensure there are issues to display, if not display notice
if (!$users = diploma_get_issues($diploma->id, $DB->sql_fullname(), $groupmode, $cm, $page, $perpage)) {
    echo $OUTPUT->header();
    groups_print_activity_menu($cm, $CFG->wwwroot . '/mod/diploma/report.php?id='.$id);
    notify(get_string('nodiplomasissued', 'diploma'));
...
if ($download == "ods") {
    ...
    // Calculate file name
    $filename = clean_filename("$course->shortname " . rtrim($diploma->name, '.') . '.ods');
    // Creating a workbook
    ...
    if ($users) {
        foreach ($users as $user) {
            ...
            $myxls->write_string($row, 5, diploma_get_grade($diploma, $course, $user->id));
...
if ($download == "xls") {
    ...
    // Calculate file name
    $filename = clean_filename("$course->shortname " . rtrim($diploma->name, '.') . '.xls');
    // Creating a workbook
...
if ($download == "txt") {
    $filename = clean_filename("$course->shortname " . rtrim($diploma->name, '.') . '.txt');
...
$usercount = count(diploma_get_issues($diploma->id, $DB->sql_fullname(), $groupmode, $cm));
...
foreach ($users as $user) {
    ...
    $date = userdate($user->timecreated) . diploma_print_user_files($diploma, $user->id, $context->id);
    $code = $user->serialnumber;
