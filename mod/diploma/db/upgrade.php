...
/**
 * This file keeps track of upgrades to the diploma module
 *
 * @package    mod
 * @subpackage diploma
...
function xmldb_diploma_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2007061300) {
        // Add new fields to diploma table
        $table = new xmldb_table('diploma');
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2007061300, 'diploma');
    }

    if ($oldversion < 2007061301) {
        $table = new xmldb_table('diploma_linked_modules');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE,  null, null);
        $table->add_field('diploma_id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0', 'id');
        $table->add_field('linkid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0', 'diploma_id');
        ...
        $table->add_index('diploma_id', XMLDB_INDEX_NOTUNIQUE, array('diploma_id'));
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2007061301, 'diploma');
    }

    if ($oldversion < 2007061302) {
        $table = new xmldb_table('diploma_linked_modules');
        $field = new xmldb_field('linkid');
        $field->set_attributes(XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'diploma_id');
        $dbman->change_field_unsigned($table, $field);

        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2007061302, 'diploma');
    }
    ...
    if ($oldversion < 2007102800) {
        ...
        $table = new xmldb_table('diploma_issues');
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2007102800, 'diploma');
    }
    ...
    if ($oldversion < 2007102806) {
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2007102806, 'diploma');
    }

    if ($oldversion < 2008080904) {
        // Add new fields to diploma table if they dont already exist
        $table = new xmldb_table('diploma');
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2008080904, 'diploma');
    }
    ...
    if ($oldversion < 2009062900) {
        // Add new field to diploma table
        $table = new xmldb_table('diploma');
        ...
        $field = new xmldb_field('orientation', XMLDB_TYPE_CHAR, '10', null, XMLDB_NOTNULL, null, ' ', 'diplomatype');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        $DB->set_field('diploma', 'orientation', 'P', array('diplomatype' => 'portrait'));
        $DB->set_field('diploma', 'orientation', 'P', array('diplomatype' => 'letter_portrait'));
        $DB->set_field('diploma', 'orientation', 'P', array('diplomatype' => 'unicode_portrait'));
        $DB->set_field('diploma', 'orientation', 'L', array('diplomatype' => 'landscape'));
        $DB->set_field('diploma', 'orientation', 'L', array('diplomatype' => 'letter_landscape'));
        $DB->set_field('diploma', 'orientation', 'L', array('diplomatype' => 'unicode_landscape'));

        // Update all the diploma types
        $DB->set_field('diploma', 'diplomatype', 'A4_non_embedded', array('diplomatype' => 'landscape'));
        $DB->set_field('diploma', 'diplomatype', 'A4_non_embedded', array('diplomatype' => 'portrait'));
        $DB->set_field('diploma', 'diplomatype', 'A4_embedded', array('diplomatype' => 'unicode_landscape'));
        $DB->set_field('diploma', 'diplomatype', 'A4_embedded', array('diplomatype' => 'unicode_portrait'));
        $DB->set_field('diploma', 'diplomatype', 'letter_non_embedded', array('diplomatype' => 'letter_landscape'));
        $DB->set_field('diploma', 'diplomatype', 'letter_non_embedded', array('diplomatype' => 'letter_portrait'));

        // savepoint reached
        upgrade_mod_savepoint(true, 2009062900, 'diploma');
    }

    if ($oldversion < 2011030105) {

        // Define field id to be added to diploma
        $table = new xmldb_table('diploma');
        $field = new xmldb_field('introformat', XMLDB_TYPE_INTEGER, '4', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, 0, 'intro');
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2011030105, 'diploma');
    }

    // The Moodle 2.0 CVS diploma version sets it to 2011110101, if the user performed an upgrade
    ...
    if ($oldversion < 2011110102) {
        ...
        // process but fresh diploma 1.9 install from CVS MOODLE_19_STABLE set the Moodle version
        ...
        $orientationfield = new xmldb_field('orientation', XMLDB_TYPE_CHAR, '10', null, XMLDB_NOTNULL, null, ' ', 'diplomatype');
        ...
        if ($dbman->table_exists('diploma_linked_modules')) {
            ...
            if ($certs = $DB->get_records('diploma')) {
                foreach ($certs as $cert) {
                    if ($cert->lockgrade == 0) {
                        // Can skip this diploma, no course grade required
                        continue;
                    }
                    if (!$cm = get_coursemodule_from_instance('diploma', $cert->id)) {
                        // Not valid skip it
                        continue;
                    }
            ...
            // Now we need to loop through the restrictions in the diploma_linked_modules
            // table and convert it into the new Moodle 2.0 restrictions
            if ($certlinks = $DB->get_records('diploma_linked_modules')) {
                foreach ($certlinks as $link) {
                    ...
                    if (!$cm = get_coursemodule_from_instance('diploma', $link->diploma_id)) {
                        // Not valid skip it
                        continue;
                    }
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2011110102, 'diploma');
    }
    ...
    if ($oldversion < 2011110103) {
        ...
        // If the diploma type does not match any of the orientations in the above then set to 'L'
        $sql = "UPDATE {diploma}
                SET orientation = 'L'
                WHERE orientation = ''";
        $DB->execute($sql);
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2011110103, 'diploma');
    }

    if ($oldversion < 2012022001) {
        // CONTRIB-3470 - certdate remaining 0 on issued diplomas, need to update
        $sql = "UPDATE {diploma_issues}
                SET certdate = timecreated
                WHERE certdate = 0";
        $DB->execute($sql);

        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2012022001, 'diploma');
    }
    ...
    if ($oldversion < 2012060901) {
        ...
        // The diploma will now always return the current grade, student name
        // and course name.
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2012060901, 'diploma');
    }
    ...
    if ($oldversion < 2012072501) {
        ...
        if ($dbman->table_exists('diploma_linked_modules')) {
            ...
            if ($certlinks = $DB->get_records('diploma_linked_modules')) {
                foreach ($certlinks as $link) {
                    ...
                    if ($link->linkid == '-1') {
                        // Make sure the diploma exists
                        if ($diploma = $DB->get_record('diploma', array('id' => $link->diploma_id))) {
                            $diploma->requiredtime = $link->linkgrade;
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2012072501, 'diploma');
    }
    ...
    if ($oldversion < 2012082401) {
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2012082401, 'diploma');
    }
    ...
    if ($oldversion < 2012090901) {
        ...
        // diploma savepoint reached
        upgrade_mod_savepoint(true, 2012090901, 'diploma');
    }
    
    if ($oldversion < 2013102308) {
    	// Add new fields to diploma table for signature improvements
    	$table = new xmldb_table('diploma');
    	
    	$field = new xmldb_field('signaturename');
    	$field->set_attributes(XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, '', 'printsignature');
    	if (!$dbman->field_exists($table, $field)) {
    		$dbman->add_field($table, $field);
    	}    	
    	
    	$field = new xmldb_field('signaturejobpossition');
    	$field->set_attributes(XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, '', 'signaturename');
    	if (!$dbman->field_exists($table, $field)) {
    		$dbman->add_field($table, $field);
    	}    	
    	
    	$field = new xmldb_field('signatureinstitution');
    	$field->set_attributes(XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, '', 'signaturejobpossition');
    	if (!$dbman->field_exists($table, $field)) {
    		$dbman->add_field($table, $field);
    	}    	
    	
    	// diploma savepoint reached.
    	upgrade_mod_savepoint(true, 2013102308, 'diploma');    	
    }
